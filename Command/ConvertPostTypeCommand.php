...
class ConvertPostTypeCommand extends ContainerAwareCommand
{
    ...
    protected function convertAcf($oldName, $newName)
    {
        $args  = array(
            'numberposts' => - 1,
            'post_type'   => 'acf',
            'post_status' => ['draft', 'publish', 'private', 'pending', 'future', 'auto-draft', 'trash']
        );
            ...
            foreach ($acfs as $acf) {
                ...
                foreach ($rules as $rule) {
                    if (isset( $rule['param'] ) &&
                        ...
                        $oldRule = $rule;
                        $rule['value'] = $newName;
                        update_post_meta($acf->ID, 'rule', $rule, $oldRule);

                        $postName = 'post_name';
                        $this->output->writeln('Updating ACF ' . $acf->{$postName});
                    }
                }
    ...
    /**
     * Finds P2P references in database and updates them
     */
    protected function fixP2P()
    {
        ...
        $brokenP2p = $wpdb->get_results("SELECT j.*, f.post_type as from_type, t.post_type as to_type
            ...
            AND j.p2p_type <> CONCAT(t.post_type, '_', f.post_type)", ARRAY_A);
